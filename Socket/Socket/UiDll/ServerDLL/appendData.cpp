...
#include <locale>
using std::get;
vector<tuple<wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring>> Database::list_of_books;
vector<tuple<wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring>>list_of_users;
...
int Database::callback(void* NotUsed, int argc, char** argv, char** azColName) 
{
	...
	for (size_t i = 0; i < argc; i++)
	{
		...
		if (i < 13)
		{
			...
			if (item == 12)
			{
				list_of_books.push_back(std::make_tuple(arr[0],arr[1],arr[2],arr[3],arr[4],arr[5], arr[6], arr[7], arr[8], arr[9], arr[10], arr[11],arr[12],arr[13]));
				item = 0;
			}
			item++;
		}
...
void Database::Select_Books(vector<tuple<wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring>>& result,string table, string item = "*", wstring condition = L"",string order="")
{
	sqlite3_stmt* res;
	list_of_books.clear();
	wstring sql2;
	...
	while (sqlite3_step(res) == SQLITE_ROW) 
	{
		...
		list_of_books.push_back(std::make_tuple(static_cast<const wchar_t*>( sqlite3_column_text16(res, 0)), static_cast<const wchar_t*>( sqlite3_column_text16(res, 1)),static_cast<const wchar_t*>( sqlite3_column_text16(res, 2)),
...
void Database::Select_Users(vector<tuple<wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring>>& result, string table, string item = "*", wstring condition = L"", string order = "")
{
	sqlite3_stmt* res;
	list_of_users.clear();
	wstring sql2;
	...
	{
		wstring a = static_cast<const wchar_t*>(sqlite3_column_text16(res, 1));
		list_of_users.push_back(std::make_tuple(static_cast<const wchar_t*>(sqlite3_column_text16(res, 0)), static_cast<const wchar_t*>(sqlite3_column_text16(res, 1)), static_cast<const wchar_t*>(sqlite3_column_text16(res, 2)),
			static_cast<const wchar_t*>(sqlite3_column_text16(res, 3)), static_cast<const wchar_t*>(sqlite3_column_text16(res, 4)), static_cast<const wchar_t*>(sqlite3_column_text16(res, 5)), static_cast<const wchar_t*>(sqlite3_column_text16(res, 6)),
			static_cast<const wchar_t*>(sqlite3_column_text16(res, 7)), static_cast<const wchar_t*>(sqlite3_column_text16(res, 8)), static_cast<const wchar_t*>(sqlite3_column_text16(res, 9)), static_cast<const wchar_t*>(sqlite3_column_text16(res, 10))));
...
int Database::decode_massage(wstring mssg ,int id_length)
{
	wstring socket_id;
	...
	if (request_type == L"[genre select]")
	{
		...
		Select_Books(result, "Books", "*", L"WHERE Genre = '" + genre + L"'", "ORDER BY ID ASC");
		wstring bookdata;
		std::ofstream file(L"respond" + socket_id + L".txt", std::ios::out | std::ios::binary);
		bookdata += socket_id + L"_[genre select]-";
		for (int i = 0; i < result.size(); i++)
		{
			bookdata += L"id=" + get<0>(result[i]) + L","
	...
	{
		vector<tuple<wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring, wstring>> result;
		req = mssg.substr(mssg.find_first_of('-') + 1, mssg.length() - 2 - mssg.find_first_of('-'));
		...
		Select_Users(result, "Users", "*", L"WHERE Email = '" + email + L"' AND Password = '" + password + L"'", "ORDER BY ID ASC");
		wstring bookdata;
		std::ofstream file(L"respond" + socket_id + L".txt", std::ios::out | std::ios::binary);
		bookdata += socket_id + L"_[sign in]-";
		if (result.size() == 0)
		{
			bookdata += L"(اطلاعات وارد شده یافت نشد . لطفا ثبت نام کنید)";
		}
		else if (get<8>(result[0]) == email && get<10>(result[0]) == password)
		{
			bookdata += L"(وارد شدید :) )";
		}
		else
		{
			bookdata += L"(اطلاعات وارد شده اشتباه است)";
		}
		//for (int i = 0; i < result.size(); i++)
		//{
		//	bookdata += L"id=" + get<0>(result[i]) + L","
		//		+ L"fname=" + get<1>(result[i]) + L","
		//		+ L"lname=" + get<2>(result[i]) + L","
		//		+ L"age=" + get<3>(result[i]) + L","
		//		+ L"gender=" + get<4>(result[i]) + L","
		//		+ L"birthdate=" + get<5>(result[i]) + L","
		//		+ L"addrres=" + get<6>(result[i]) + L","
		//		+ L"postalcode=" + get<7>(result[i]) + L","
		//		+ L"email=" + get<8>(result[i]) + L","
		//		+ L"username=" + get<9>(result[i]) + L","
		//		+ L"password=" + get<10>(result[i]) + L","
		//		+ L"[[[End]]]\n";
